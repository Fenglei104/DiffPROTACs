...
class EGCL_EH(nn.Module):
    """
    m_ij = phi_e(h_i,h_j,d,a)
    h_i = phi_h(h_i,sum(e*m_ij))
    """
...
class EGCL_X(nn.Module):
    """
    x_i = x_i + sum((x_i-x_j)*phi_x(h_i,h_j,d,a)/(d_ij+1)) 
    """
    def __init__(self, hidden_nf, normalization_factor, aggregation_method,
                 edges_in_d=1, activation=nn.SiLU(), tanh=False, coords_range=10.0):
        super(EGCL_X, self).__init__()
        self.tanh = tanh
...
class EGCL(nn.Module):
    def __init__(self, hidden_nf, 
                 edge_feat_nf=2, activation=nn.SiLU(), n_layers=2, attention=True,
        ...
        for i in range(0, n_layers):
            self.add_module("egcl_eh%d" % i, EGCL_EH(
                self.hidden_nf, self.hidden_nf, self.hidden_nf, edges_in_d=edge_feat_nf,
                activation=activation, attention=attention,
                normalization_factor=self.normalization_factor,
                aggregation_method=self.aggregation_method))
        self.add_module("egcl_x", EGCL_X(
            hidden_nf, edges_in_d=edge_feat_nf, activation=activation, tanh=tanh,
    ...
    def forward(self, h, x, edge_index, node_mask=None, linker_mask=None, edge_mask=None, edge_attr=None):
        ...
        distances, coord_diff = utils.coord2diff(x, edge_index, self.norm_constant)
        if self.sin_embedding is not None:
            distances = self.sin_embedding(distances)
        edge_attr = torch.cat([distances, edge_attr], dim=1)
        for i in range(0, self.n_layers):
            h, _ = self._modules["egcl_eh%d" % i](h, edge_index, edge_attr=edge_attr, node_mask=node_mask, edge_mask=edge_mask)
        x = self._modules["egcl_x"](
            h, x,
...
class EGNN(nn.Module):
    def __init__(self, in_node_nf, in_edge_nf, hidden_nf, 
                 activation=nn.SiLU(), n_layers=3, attention=False, norm_diff=True, 
        ...
        for i in range(0, n_layers):
            self.add_module("egcl_%d" % i, EGCL(hidden_nf, edge_feat_nf=edge_feat_nf,
    ...
    def forward(self, h, x, edge_index, node_mask=None, linker_mask=None, edge_mask=None):
        # Edit Emiel: Remove velocity as input
        distances, _ = utils.coord2diff(x, edge_index)
        if self.sin_embedding is not None:
            distances = self.sin_embedding(distances)

        h = self.embedding(h)
        for i in range(0, self.n_layers):
            h, x = self._modules["egcl_%d" % i](
                h, x, edge_index,
                node_mask=node_mask,
                linker_mask=linker_mask,
                edge_mask=edge_mask,
...
class GNN(nn.Module):
    ...
    def forward(self, h, edges, edge_attr=None, node_mask=None, edge_mask=None):
        ...
        h = self.embedding(h)
        for i in range(0, self.n_layers):
            h, _ = self._modules["egcl_eh%d" % i](h, edges, edge_attr=edge_attr, node_mask=node_mask, edge_mask=edge_mask)
        h = self.embedding_out(h)
